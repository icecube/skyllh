...
class SignalTimePDFTestCase(
    ...
    def setUp(self):
        ...
        self.pmm = ParameterModelMapper(models=[SourceModel(), SourceModel()])
        ...
        self.time_flux_profile = BoxTimeFluxProfile(t0=5, tw=10, cfg=self.cfg)

        self.sig_time_pdf = SignalTimePDF(
            pmm=self.pmm, livetime=self.livetime, time_flux_profile=self.time_flux_profile, cfg=self.cfg
        )
    ...
    def test_get_pd(self):
        ...
        src_params_recarray = self.pmm.create_src_params_recarray(gflp_values=np.array([]))
