...
class TestMultiDatasetSignalGeneratorEnergyRangeConsistency(unittest.TestCase):
    ...
    def _make_sig_gen(configured_ranges):
        ...
        sig_gen._shg_mgr = _DummyShgMgr(n_sources=2)  # pyright: ignore[reportAttributeAccessIssue]
        ...
        sig_gen._ds_sig_weight_factors_service = _DummyDatasetSignalWeightFactorsService(src_service=src_service)  # pyright: ignore[reportAttributeAccessIssue]
...
class TestMultiDatasetSignalGeneratorKwargPolicy(unittest.TestCase):
    def test_generate_signal_events_rejects_energy_range_kwarg(self):
        ...
        sig_gen._ds_sig_weight_factors_service = _DummyDatasetSignalWeightFactorsService(  # pyright: ignore[reportAttributeAccessIssue]
        ...
        with self.assertRaises(TypeError):
            sig_gen.generate_signal_events(rss=rss, mean=10, poisson=False, energy_range=(1e2, 1e3))  # pyright: ignore[reportCallIssue]
