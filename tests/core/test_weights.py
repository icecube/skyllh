...
class TestSourceDetectorWeights(unittest.TestCase):
    ...
    def test_without_grads(self):
        ...
        detsigyield_arr = np.array([[SimpleDetSigYieldWithoutGrads()], [SimpleDetSigYieldWithoutGrads()]])

        detsigyield_service = create_DetSigYieldService(shg_mgr=type(self)._shg_mgr, detsigyield_arr=detsigyield_arr)

        src_detsigyield_weights_service = SrcDetSigYieldWeightsService(detsigyield_service=detsigyield_service)
        ...
        np.testing.assert_allclose(
            a_jk, np.array([[1 * 10, 2 * 20, 3 * 30], [1 * 10, 2 * 20, 3 * 30]]), err_msg='a_jk values'
        )
    ...
    def test_with_grads_p1(self):
        ...
        """
        # detsigyield_arr is (N_datasets, N_shgs)-shaped.
        detsigyield_arr = np.array([[SimpleDetSigYieldWithGrads(pname='p1')], [SimpleDetSigYieldWithGrads(pname='p1')]])
        ...
        np.testing.assert_allclose(a_jk, [[1 * 10, 2 * 20, 3 * 30], [1 * 10, 2 * 20, 3 * 30]], err_msg='a_jk values')
        ...
        np.testing.assert_allclose(
            a_jk_grads[0],
            [[1 * 120.0, 2 * 120.0, 3 * 120.0], [1 * 120.0, 2 * 120.0, 3 * 120.0]],
    ...
    def test_with_grads_p2(self):
        ...
        """
        # detsigyield_arr is (N_datasets, N_shgs)-shaped.
        detsigyield_arr = np.array([[SimpleDetSigYieldWithGrads(pname='p2')], [SimpleDetSigYieldWithGrads(pname='p2')]])
        ...
        np.testing.assert_allclose(
            a_jk_grads[1],
            [[1 * 177.0, 2 * 177.0, 3 * 177.0], [1 * 177.0, 2 * 177.0, 3 * 177.0]],
...
class TestDatasetSignalWeightFactors(unittest.TestCase):
    ...
    def test_without_grads(self):
        ...
        """
        # detsigyield_arr is (N_datasets, N_shgs)-shaped.
        detsigyield_arr = np.array([[SimpleDetSigYieldWithoutGrads(scale=1)], [SimpleDetSigYieldWithoutGrads(scale=2)]])
    ...
    def test_with_grads_p1(self):
        ...
        detsigyield_arr = np.array(
            [[SimpleDetSigYieldWithGrads(pname='p1', scale=1)], [SimpleDetSigYieldWithGrads(pname='p1', scale=2)]]
        )
        ...
        np.testing.assert_allclose(f_j_grads[0], [0.57142857, -0.57142857], err_msg='f_j_grads[0] values')
    ...
    def test_with_grads_p2(self):
        ...
        # detsigyield_arr is (N_datasets, N_shgs)-shaped.
        detsigyield_arr = np.array(
            [[SimpleDetSigYieldWithGrads(pname='p2', scale=1)], [SimpleDetSigYieldWithGrads(pname='p2', scale=2)]]
        )
        ...
        np.testing.assert_allclose(f_j_grads[1], [0.84619048, -0.84619048], err_msg='f_j_grads[1] values')
