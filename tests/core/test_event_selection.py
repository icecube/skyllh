...
def shgm_setup(n_sources=1):
    ...
    source_list = [PointLikeSource(*src_loc) for src_loc in zip(src_ras, src_decs, strict=True)]
...
class AllEventSelectionMethod_TestCase(unittest.TestCase):
    ...
    def test_select_events_single_source(self):
        ...
        np.testing.assert_array_equal(np.unique(ev_idxs), np.arange(len(self.test_events)))
...
class DecBandEventSectionMethod_TestCase(unittest.TestCase):
    ...
    def test_select_events_single_source(self):
        ...
        n_expected_events = np.sum((events['dec'] > dec_min) & (events['dec'] < dec_max))
        self.assertEqual(len(src_idxs), n_expected_events)
        self.assertEqual(len(evt_idxs), n_expected_events)
        np.testing.assert_array_equal(np.unique(src_idxs), np.arange(n_sources))
        np.testing.assert_array_equal(np.unique(evt_idxs), np.arange(len(events)))
    ...
    def test_select_events_multiple_sources(self):
        ...
        for i, src_dec in enumerate(src_decs):
            ...
            self.assertTrue(
                np.all(events['dec'][evt_idxs[events_mask]] > dec_min),
                msg='Returned selected events below src_dec - delta_angle.',
            )
            self.assertTrue(
...
class RABandEventSectionMethod_TestCase(unittest.TestCase):
    ...
    def test_select_events_single_source(self):
        ...
        for i in range(n_sources):
            ...
            self.assertTrue(
                np.all(np.fabs(events['ra'][ev_idxs[events_mask]] - np.pi) < src_ra_max),
                msg='Returned selected events above maximal src_ra + delta_angle/cosfact.',
            )
        ...
        np.testing.assert_array_equal(np.unique(src_idxs), np.arange(n_sources))
        np.testing.assert_array_equal(np.unique(ev_idxs), np.arange(len(events)))
    ...
    def test_select_events_multiple_sources(self):
        ...
        for i in range(n_sources):
            ...
            self.assertTrue(
                np.all(np.fabs(events['ra'][evt_idxs[events_mask]] - np.pi) < src_ra_max),
                msg='Returned selected events above maximal src_ra + delta_angle/cosfact.',
            )
...
class AngErrOfPsiAndSpatialBoxEventSelectionMethod_TestCase(unittest.TestCase):
    ...
    def test_select_events_single_source(self):
        ...
        evt_sel_method_angerr = AngErrOfPsiEventSelectionMethod(shg_mgr=shg_mgr, func=func, psi_floor=0.0)

        evt_sel_method_sb = SpatialBoxEventSelectionMethod(shg_mgr=shg_mgr, delta_angle=delta_angle)
    ...
    def test_select_events_multiple_sources_psi_func(self):
        ...
        for i in range(n_sources):
            ...
            mask_psi_cut = evts_sb[evt_idxs_sb[evts_mask_sb]]['ang_err'] > ang_err

            np.testing.assert_array_equal(
                evts[evt_idxs[evts_mask]].as_numpy_record_array(),
                evts_sb[evt_idxs_sb[evts_mask_sb]][mask_psi_cut].as_numpy_record_array(),
            )
