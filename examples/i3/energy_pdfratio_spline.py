...
from skyllh.core.parameters import (
    make_linear_parameter_grid_1d,
)
from skyllh.i3.backgroundpdf import (
    DataBackgroundI3EnergyPDF,
)
from skyllh.i3.pdfratio import (
    I3EnergySigOverBkgPDFRatioSpline,
)
from skyllh.i3.signalpdf import (
    SignalI3EnergyPDF,
)
from skyllh.physics.flux import (
    PowerLawFlux,
)
from skyllh.plotting.i3.pdfratio import (
    I3EnergySigSetOverBkgPDFRatioSplinePlotter,
)
...
GFU_dsc = datasample.create_dataset_collection(
    "/home/mwolf/data/ana/analyses/gfu/version-002-p01/"
)
...
fluxmodel = PowerLawFlux(Phi0=1.0, E0=1e3, gamma=2.0)

# Define the fit parameter grid for the gamma fit parameter.
gamma_param_grid = make_linear_parameter_grid_1d(
    name="gamma", low=1.0, high=4.0, delta=0.1
)
...
signalpdfset = SignalI3EnergyPDF(
    ds.data_mc,
    logE_binning,
    sinDec_binning,
    fluxmodel,
    fitparams_grid_set=gamma_param_grid,
)

# Create an IceCube specific energy background PDF object from experimental
# data.
backgroundpdf = DataBackgroundI3EnergyPDF(
    ds.data_exp, logE_binning, sinDec_binning
)
