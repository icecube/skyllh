...
\begin{equation}
\mathcal{R}_i(\ps) = \frac{\mathcal{S}_i(\ps)}{\mathcal{B}_i} = \frac{1}{A(\ps)} \sum_{k=1}^{K} a_k(\psk) \mathcal{R}_{i,k}(\psk),
\label{eq:RiStacking}
\end{equation}
where the signal over background PDF ratio
$\mathcal{R}_{i,k}(\psk) \equiv \mathcal{S}_{i}(\psk) / \mathcal{B}_{i}$
...
with
\begin{equation}
\frac{\mathrm{d} a_k}{\mathrm{d} p_{\mathrm{s}}} = W_k \frac{\mathrm{d}\mathcal{Y}_{\mathrm{s}_{k}}(\psk)}{\mathrm{d}p_{\mathrm{s}}}.
\end{equation}
