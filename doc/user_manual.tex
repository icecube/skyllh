...
\end{equation}
In case of a single source, the expression for the derivative of the sample
weight factor, where $f_j(\ps)$ is given by equation (\ref{eq:sample-weight-factor}),
reads via the quotient rule of differentation:
\begin{equation}
\frac{\mathrm{d}f_j(\ps)}{\mathrm{d}p_{\mathrm{s}}} = \frac{\frac{\mathrm{d}\mathcal{Y}_{\mathrm{s},j}(\ps)}{\mathrm{d}p_{\mathrm{s}}} \sum_{j'=1}^{J}\mathcal{Y}_{\mathrm{s},j'}(\ps) - \mathcal{Y}_{\mathrm{s},j}(\ps) \sum_{j'=1}^{J} \frac{\mathrm{d}\mathcal{Y}_{\mathrm{s},j'}(\ps)}{\mathrm{d}p_{\mathrm{s}}} }{\left( \sum_{j'=1}^{J} \mathcal{Y}_{\mathrm{s},j'}(\ps) \right)^2}
\end{equation}
