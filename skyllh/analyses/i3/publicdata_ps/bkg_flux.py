...
def get_flux_astro_decnu_log10enu(decnu_binedges: np.ndarray, log10_enu_binedges: np.ndarray) -> np.ndarray:
    ...
    """
    fluxmodel = SteadyPointlikeFFM(Phi0=1.44e-18, energy_profile=PowerLawEnergyFluxProfile(E0=100e3, gamma=2.37))
...
def convert_flux_bkg_to_pdf_bkg(
    ...
    if not np.isclose(np.sum(p_bkg * bin_area), 1):
        raise ValueError(f'The background PDF is not normalized! The integral is {np.sum(p_bkg * bin_area)}!')
...
def get_pd_atmo_decnu_Enu(
    ...
    if not np.isclose(np.sum(pd_atmo * bin_area), 1):
        raise ValueError(
            f'The atmospheric true energy PDF is not normalized! The integral is {np.sum(pd_atmo * bin_area)}!'
        )
...
def get_pd_atmo_E_nu_sin_dec_nu(flux_pathfilename: str) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    if not np.all(np.isclose(np.sum(pd_atmo * dE[np.newaxis, :], axis=1), 1)):
        raise ValueError('The atmospheric true energy PDF is not normalized!')
...
def get_pd_astro_E_nu_sin_dec_nu(sin_dec_binedges: np.ndarray, log10_e_grid_edges: np.ndarray) -> np.ndarray:
    ...
    if not np.all(np.isclose(np.sum(pd_astro * dE[np.newaxis, :], axis=1), 1)):
        raise ValueError('The astrophysical energy PDF is not normalized!')
...
def get_pd_bkg_E_nu_sin_dec_nu(pd_atmo: np.ndarray, pd_astro: np.ndarray, log10_e_grid_edges: np.ndarray) -> np.ndarray:
    ...
    if not np.all(np.isclose(np.sum(pd_bkg * dE[np.newaxis, :], axis=1), 1)):
        raise ValueError('The background energy PDF is not normalized!')
