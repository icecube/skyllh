...
class PDSignalEnergyPDF(
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not isinstance(f_e_spl, FctSpline1D):
            raise TypeError(
                f'The f_e_spl argument must be an instance of FctSpline1D! Its current type is {classname(f_e_spl)}!'
            )
    ...
    def get_pd_by_log10_reco_e(self, log10_reco_e, tl=None):
        ...
        m = (log10_reco_e >= self.log10_reco_e_min) & (log10_reco_e < self.log10_reco_e_max)
...
class PDSignalEnergyPDFSet(
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if (not isinstance(param_grid_set, ParameterGrid)) and (not isinstance(param_grid_set, ParameterGridSet)):
            raise TypeError(
        ...
        (min_log_true_e, max_log_true_e) = sm.get_true_log_e_range_with_valid_log_e_pdfs(true_dec_idx)
        log_true_e_mask = np.logical_and(
            sm.log10_true_enu_binedges >= min_log_true_e, sm.log10_true_enu_binedges <= max_log_true_e
        )
        true_enu_binedges = np.power(10, sm.log10_true_enu_binedges[log_true_e_mask])
        ...
        self._logger.debug(f'dE_nu = {d_enu}')

        # Load the effective area.
        aeff = PDAeff(pathfilenames=ds.get_abs_pathfilename_list(ds.get_aux_data_definition('eff_area_datafile')))
        ...
        def create_energy_pdf(sm_pdf_dec_slice, fluxmodel, gridparams):
            ...
            sum_pdf = np.sum(
                [create_reco_e_pdf_for_true_e(i, true_e_idx) for (i, true_e_idx) in enumerate(valid_true_e_idxs)],
                axis=0,
            )
        ...
        args_list = [((sm_pdf_dec_slice, fluxmodel, gridparams), {}) for gridparams in self.gridparams_list]

        pdf_list = parallelize(create_energy_pdf, args_list, ncpu=self.ncpu, ppbar=ppbar)
        ...
        for gridparams, pdf in zip(self.gridparams_list, pdf_list, strict=True):
