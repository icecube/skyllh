...
class PDSignalEnergyPDF(PDF, IsSignalPDF):
    ...
    def __init__(self, f_e_spl, **kwargs):
    ...
    def get_pd_by_log10_reco_e(self, log10_reco_e, tl=None):
        ...
        m = (log10_reco_e >= self.log10_reco_e_min) & (
    ...
    def __init__(
        ...
        **kwargs,
        ...
        (
            min_log_true_e,
            max_log_true_e,
        ) = sm.get_true_log_e_range_with_valid_log_e_pdfs(true_dec_idx)
        ...
        self._logger.debug("dE_nu = {}".format(d_enu))
        ...
        self._logger.debug(
            "det_prob = {}, sum = {}".format(det_prob, np.sum(det_prob))
        )
        ...
        def create_energy_pdf(sm_pdf, fluxmodel, gridparams):
            ...
            flux_prob = my_fluxmodel.energy_profile.get_integral(
                ...
                true_enu_binedges[0], true_enu_binedges[-1]
            )
            ...
            self._logger.debug(
                "flux_prob = {}, sum = {}".format(flux_prob, np.sum(flux_prob))
            )
            ...
            self._logger.debug("true_e_prob = {}".format(true_e_prob))
        ...
        pdf_list = parallelize(
            create_energy_pdf, args_list, ncpu=self.ncpu, ppbar=ppbar
        )
