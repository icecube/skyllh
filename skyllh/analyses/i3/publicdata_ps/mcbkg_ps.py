...
def create_analysis(
    ...
    for ds_idx, ds in enumerate(datasets):
        ...
        data = ds.load_and_prepare_data(
            keep_fields=keep_data_fields, compress=compress_data, efficiency_mode=efficiency_mode, tl=tl
        )

        sin_dec_binning = ds.get_binning_definition('sin_dec')

        # Create the spatial PDF ratio instance for this dataset.
        spatial_sigpdf = RayleighPSFPointSourceSignalSpatialPDF(dec_range=np.arcsin(sin_dec_binning.range))
...
if __name__ == '__main__':
    ...
    p.add_argument('--dec', default=23.8, type=float, help='The source declination in degrees.')
    p.add_argument('--ra', default=216.76, type=float, help='The source right-ascention in degrees.')
    p.add_argument('--gamma-seed', default=3, type=float, help='The seed value of the gamma fit parameter.')
    p.add_argument('--data_base_path', default=None, type=str, help='The base path to the data samples (default=None)')
    p.add_argument(
        '--seed', default=1, type=int, help='The random number generator seed for the likelihood minimization.'
    )
    p.add_argument(
        '--ncpu', default=1, type=int, help='The number of CPUs to utilize where parallelization is possible.'
    )
    args = p.parse_args()
    ...
    setup_file_handler('skyllh', 'debug.log', log_level=logging.DEBUG, log_format=log_format)
    ...
    with tl.task_timer('Creating analysis.'):
        ana = create_analysis(datasets=datasets, source=source, gamma_seed=args.gamma_seed, tl=tl)

    with tl.task_timer('Unblinding data.'):
        (TS, fitparam_dict, status) = ana.unblind(rss)

    print(f'TS = {TS}')
    print(f'ns_fit = {fitparam_dict["ns"]}')
    print(f'gamma_fit = {fitparam_dict["gamma"]}')
