...
def create_analysis(
        ...
        astro_diff_bkg_file_list='default',
    ...
    astro_diff_bkg_file_list : {'default, list of file names}
        The list with the names of the files of the 2D histogram with 
        the number of expected muons from the astrophisical diffuse background. 
        'Default' loads the precomputed values for that dataset. 
        See analysis/i3/publicdata_ps/astro_bkg_flux folder for more details.
        Used only for numerical_stabilizer='physical'
    minimizer_impl : str
    ...
    if astro_diff_bkg_file_list == 'default':
        astro_diff_bkg_file_list = ['default'] * len(datasets)
...
#==============================================================================
    ...
    for (ds_idx, ds) in enumerate(datasets):
        ...
        energy_bkgpdf = PDDataBackgroundI3EnergyPDF(
            ...
            astro_diff_bkg_file=astro_diff_bkg_file_list[ds_idx])
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        default=[5.7],
        type=float,
        help='The source(s) declination in degrees.'
    )
    parser.add_argument(
        '--ra-list',
        dest='ra_list',
        default=[77.35],
        type=float,
        help='The source(s) right-ascension in degrees.'
    )
    parser.add_argument(
        '--gamma-seed',
        dest='gamma_seed',
    ...
    for i in range(len(args.dec_list)):
        catalog += PointLikeSource(
            name=f'My Point-Like-Source {i}',
            ra=np.deg2rad(args.ra_list[i]),
            dec=np.deg2rad(args.dec_list[i]))
    ...
    with tl.task_timer('Creating analysis.'):
        ana = create_analysis(
            ...
            numerical_stabilizer=args.numerical_stabilizer,
