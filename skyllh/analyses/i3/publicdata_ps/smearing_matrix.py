...
def load_smearing_histogram(
    ...
    reco_e_lower_edges = np.reshape(data['e_min'][idxs], (n_true_e, n_true_dec, n_reco_e))
    ...
    psi_lower_edges = np.reshape(data['psi_min'][idxs], (n_true_e, n_true_dec, n_reco_e, n_psi))
    psi_upper_edges = np.reshape(data['psi_max'][idxs], (n_true_e, n_true_dec, n_reco_e, n_psi))

    # Get angular error bin_edges as a 5d array.
    ang_err_lower_edges = np.reshape(data['ang_err_min'], (n_true_e, n_true_dec, n_reco_e, n_psi, n_ang_err))
    ang_err_upper_edges = np.reshape(data['ang_err_max'], (n_true_e, n_true_dec, n_reco_e, n_psi, n_ang_err))

    # Create 5D histogram for the probabilities.
    histogram = np.reshape(data['norm_counts'], (n_true_e, n_true_dec, n_reco_e, n_psi, n_ang_err))
...
class PDSmearingMatrix:
    ...
    def true_dec_bin_centers(self):
        ...
        """
        return 0.5 * (self._true_dec_bin_edges[:-1] + self._true_dec_bin_edges[1:])
    ...
    def get_true_dec_idx(self, true_dec: float) -> int:
        ...
        if (true_dec < self.true_dec_bin_edges[0]) or (true_dec > self.true_dec_bin_edges[-1]):
            raise ValueError(f'The declination {true_dec} degrees is not supported by the smearing matrix!')

        true_dec_idx = np.digitize(true_dec, self.true_dec_bin_edges) - 1

        return true_dec_idx

    def get_log10_true_e_idx(self, log10_true_e: float) -> int:
    ...
    def get_ang_err_idx(
        ...
        """
        lower_edges = self.ang_err_lower_edges[true_e_idx, true_dec_idx, reco_e_idx, psi_idx]
        upper_edges = self.ang_err_upper_edges[true_e_idx, true_dec_idx, reco_e_idx, psi_idx]
    ...
    def get_psi_pdf(
        ...
        lower_bin_edges = self.psi_lower_edges[log_true_e_idx, dec_idx, log_e_idx]
        upper_bin_edges = self.psi_upper_edges[log_true_e_idx, dec_idx, log_e_idx]
        bin_widths = upper_bin_edges - lower_bin_edges
    ...
    def get_ang_err_pdf(
        ...
        lower_bin_edges = self.ang_err_lower_edges[log_true_e_idx, dec_idx, log_e_idx, psi_idx]
        upper_bin_edges = self.ang_err_upper_edges[log_true_e_idx, dec_idx, log_e_idx, psi_idx]
        bin_widths = upper_bin_edges - lower_bin_edges
    ...
    def sample_log_e(
        ...
        for b_log_true_e_idx in unique_log_true_e_idxs:
            ...
            b_log_e_idx = rss.random.choice(np.arange(len(pdf)), p=(pdf * bin_widths), size=b_size)
            b_log_e = rss.random.uniform(low_bin_edges[b_log_e_idx], up_bin_edges[b_log_e_idx], size=b_size)
    ...
    def sample_psi(
        ...
        if len(log_true_e_idxs) != len(log_e_idxs):
            raise ValueError('The lengths of log_true_e_idxs and log_e_idxs must be equal!')
        ...
        for b_log_true_e_idx in unique_log_true_e_idxs:
            ...
            for bb_log_e_idx in bb_unique_log_e_idxs:
                ...
                (pdf, low_bin_edges, up_bin_edges, bin_widths) = self.get_psi_pdf(
                ...
                bb_psi_idx = rss.random.choice(np.arange(len(pdf)), p=(pdf * bin_widths), size=bb_size)
                bb_psi = rss.random.uniform(low_bin_edges[bb_psi_idx], up_bin_edges[bb_psi_idx], size=bb_size)
    ...
    def sample_ang_err(
        ...
        if (len(log_true_e_idxs) != len(log_e_idxs)) and (len(log_e_idxs) != len(psi_idxs)):
            raise ValueError('The lengths of log_true_e_idxs, log_e_idxs, and psi_idxs must be equal!')
        ...
        for b_log_true_e_idx in unique_log_true_e_idxs:
            ...
            for bb_log_e_idx in bb_unique_log_e_idxs:
                ...
                for bbb_psi_idx in bbb_unique_psi_idxs:
                    ...
                    (pdf, low_bin_edges, up_bin_edges, bin_widths) = self.get_ang_err_pdf(
                    ...
                    bbb_ang_err_idx = rss.random.choice(np.arange(len(pdf)), p=(pdf * bin_widths), size=bbb_size)
                    bbb_ang_err = rss.random.uniform(
                        low_bin_edges[bbb_ang_err_idx], up_bin_edges[bbb_ang_err_idx], size=bbb_size
                    )
