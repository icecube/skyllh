...
class PDDatasetSignalGenerator(
    ...
    def __init__(
        ...
        # Get the right-ascention and declination for each source.
...
class PDDatasetSignalGeneratorOLD(
        object):
        ...
        for (ds_idx, w) in enumerate(ds_weights):
            ...
            for shg in self._shg_mgr.shg_list:
                for (shg_src_idx, src) in enumerate(shg.source_list):
                    ...
                    sig_gen = PDDatasetSignalGeneratorOLD(
                        ds,
                        src.dec,
                        self.effA[ds_idx],
                        self.sm[ds_idx])
