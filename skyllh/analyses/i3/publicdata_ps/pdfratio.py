...
class PDSigSetOverBkgPDFRatio(SigSetOverBkgPDFRatio):
    def __init__(self, sig_pdf_set, bkg_pdf, cap_ratio=False, **kwargs):
        ...
        if self.cap_ratio:
            ...
            for sig_pdf_key in sig_pdf_set.pdf_keys:
                ...
                val = np.percentile(r[r > 1.0], ratio_perc)
    ...
    def _get_ratio_values(self, tdm, eventdata, gridparams_recarray, n_values):
        ...
        if len(gridparams_recarray) > 1:
            for pname in gridparams_recarray.dtype.fields.keys():
                if not np.all(
                    np.isclose(np.diff(gridparams_recarray[pname]), 0)
                ):
    ...
    def _calculate_ratio_and_grads(
        self, tdm, src_params_recarray, fitparams_hash
    ):
        ...
        (ratio, grads) = self._interpolmethod(
            tdm=tdm, eventdata=None, params_recarray=src_params_recarray
        )
