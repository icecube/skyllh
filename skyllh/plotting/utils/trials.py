...
def plot_ns_fit_vs_mean_ns_inj(
    ...
    ax[0].plot(
        x_bins, x_bins, color="black", alpha=0.4, linestyle="-", linewidth=2
    )

    # Plot the lower quantile.
    ax[0].plot(
        mean_n_sig, ns_fit_lq, color=line_color, linestyle="-.", linewidth=2
    )
...
def plot_gamma_fit_vs_mean_ns_inj(
    ...
    ax[0].plot(
        mean_n_sig, gamma_fit_uq, color=line_color, linestyle="-.", linewidth=2
    )
    ...
    ax[0].plot(
        mean_n_sig, gamma_fit_lq, color=line_color, linestyle="-.", linewidth=2
    )
    ...
    ax[1].plot(
        mean_n_sig[m],
        (gamma_fit_median[m] - gamma_inj) / gamma_inj * 100,
        linewidth=2,
    )
    ax[1].fill_between(
        mean_n_sig[m],
    ...
    ratio_ylabel = (
        r"$\frac{<%s> - %s}{%s}$" % (ylabel, gamma_inj_label, gamma_inj_label)
        + " [%]"
    )
    ax[1].set_ylabel(ratio_ylabel, fontsize=axis_fontsize)
    ax[1].set_xlim(x_bins[0], x_bins[-1])
