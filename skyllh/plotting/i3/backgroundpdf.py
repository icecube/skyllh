...
class BackgroundI3SpatialPDFPlotter:
    ...
    def plot(self, src_hypo_group_manager: SourceHypoGroupManager, pmm: ParameterModelMapper, axes: Axes):
        ...
        events = DataFieldRecordArray(np.zeros((pdfprobs.size,), dtype=[('sin_dec', np.float64)]))
        ...
        (left, right, bottom, top) = (
            ra_axis.vmin,
            ra_axis.vmax,
            sin_dec_binning.lower_edge,
            sin_dec_binning.upper_edge,
        )
        img = axes.imshow(
            pdfprobs.T, extent=(left, right, bottom, top), origin='lower', norm=LogNorm(), interpolation='none'
        )
        axes.set_xlabel('ra')
