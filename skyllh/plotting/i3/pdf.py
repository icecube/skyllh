...
class I3EnergyPDFPlotter(object):
    ...
    def plot(self, src_hypo_group_manager, axes, **kwargs):
        ...
        pdf_values = np.zeros(
            (xbinning.nbins, ybinning.nbins), dtype=np.float64
        )
        events = DataFieldRecordArray(
            np.zeros(
                ...
                dtype=[
                    ("ix", np.int64),
                    (xbinning.name, np.float64),
                    ("iy", np.int64),
                    (ybinning.name, np.float64),
                ],
            )
        )
        for i, ((ix, x), (iy, y)) in enumerate(
            itertools.product(
                enumerate(xbinning.bincenters), enumerate(ybinning.bincenters)
            )
        ):
            events["ix"][i] = ix
