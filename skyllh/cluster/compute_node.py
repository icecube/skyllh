...
class ComputeNode:
    ...
    def __init__(self, live_time, master_addr, master_port):
        ...
        if not reply.is_same_as(ACK):
            raise RuntimeError('The master node did not reply with an ACK command!')
    ...
    def master_port(self, p):
        p = int_cast(p, 'The master_port property must be castable to type int!')
        self._master_port = p
...
if __name__ == '__main__':
    ...
    parser.add_argument('master_addr', type=str, help='The address (IP / hostname) of the SkyLLH master program.')
    parser.add_argument('master_port', type=int, default=9999, help='The port number of the SkyLLH master program.')
    parser.add_argument(
        '--live-time', type=int, default=2 * 60 * 60, help='The time in seconds to run this compute node instance.'
    )

    args = parser.parse_args()

    cn = ComputeNode(live_time=args.live_time, master_addr=args.master_addr, master_port=args.master_port)
