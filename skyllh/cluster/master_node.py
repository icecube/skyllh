...
class MasterNode:
    ...
    def clear_cn_registry(self):
        ...
        for _, cn in self.cn_registry.items():
            cn.sock.close()
    ...
    def register_compute_nodes(self, n_cn=10, master_port=9999, blocksize=2048):
        ...
        try:
            while len(self._cn_registry) < n_cn:
                ...
                cmd = receive_command_from_socket(clientsock, blocksize=blocksize)
                if not cmd.is_same_as(RegisterCN):
                    raise RuntimeError(f'The compute node provided an unknown command "{cmd.as_message().msg}"!')
                ACK().send(clientsock)

                cn = CNRegistryEntry(clientsock, addr, port, cmd.cn_start_time, cmd.cn_live_time)
    ...
    def shutdown_compute_nodes(self):
        ...
        for _, cn in self._cn_registry.items():
