...
class ProgressBar:
    ...
    def __init__(self, maxval, startval=0, parent=None, **kwargs):
        ...
        if (self._parent is None) and self.is_shown:
            self._tqdm = tqdm(total=maxval, initial=startval, leave=True, position=0, **kwargs)
    ...
    @maxval.setter
    def maxval(self, v):
        v = int_cast(v, 'The maxval property must be castable to an integer value!')
        self._maxval = v
    ...
    @startval.setter
    def startval(self, v):
        v = int_cast(v, 'The startval property must be castable to an integer value!')

        if v >= self._maxval:
            raise ValueError(
                f'The startval value ({v}) must be smaller than the value of the `maxval` property ({self._maxval})!'
            )
    ...
    def parent(self, pbar):
        if pbar is not None and not isinstance(pbar, ProgressBar):
            raise TypeError('The parent property must be None, or an instance of ProgressBar!')
        self._parent = pbar
    ...
    def add_sub_progress_bar(self, pbar):
        ...
        if not isinstance(pbar, ProgressBar):
            raise TypeError('The pbar argument must be an instance of ProgressBar!')
