...
def parallelize(func, args_list, ncpu, rss=None, tl=None):
    ...
    for proc in processes:
        ...
        if(tl is not None):
            tl.join(proc_tl)
        logger.debug('Started handling log records of Process - %d', pid)
        lqueue_end = False
        while not lqueue_end:
            try:
                ...
                logger.debug('An empty queue exception before getting the last '
                             'queue item - None.')
        logger.debug('Finished handling log records of Process - %d', pid)
