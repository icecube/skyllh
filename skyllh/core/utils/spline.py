...
def make_spline_1d(x, y, kind='linear', **kwargs):
    ...
    **kwargs
    ...
    xy = np.array(sorted(zip(x, y, strict=True)), dtype=y.dtype)
    x = xy[:, 0]
...
class CatmullRomRegular1DSpline:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if len(x) != len(y):
            raise ValueError(f'The number of x ({len(x)}) and y ({len(y)}) data values need to be equal!')
        if len(x) < 4:
            raise ValueError(f'The number of data points ({len(x)}) must be at least 4!')
    ...
    def _eval_for_valid_x(
        ...
        np.floor((x - self._x_start) / self._delta_x, out=seg_idxs, casting='unsafe')
        m = x == self._x_stop
    ...
    def _calc_segment_coefficients(
        ...
        """
        t0 = 0
        t1 = self._calc_tj(ti=t0, Pi_x=Px[0], Pi_y=Py[0], Pj_x=Px[1], Pj_y=Py[1])
        t2 = self._calc_tj(ti=t1, Pi_x=Px[1], Pi_y=Py[1], Pj_x=Px[2], Pj_y=Py[2])
        t3 = self._calc_tj(ti=t2, Pi_x=Px[2], Pi_y=Py[2], Pj_x=Px[3], Pj_y=Py[3])
