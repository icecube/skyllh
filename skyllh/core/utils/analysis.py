...
def calculate_pval_from_trials(ts_vals, ts_threshold, comp_operator="greater"):
...
def calculate_pval_from_gammafit_to_trials(
    ts_vals, ts_threshold, eta=3.0, n_max=500000
):
...
def calculate_pval_from_trials_mixed(
    ...
    if ts_threshold < switch_at_ts:
        return calculate_pval_from_trials(
            ts_vals, ts_threshold, comp_operator=comp_operator
        )
    else:
        return calculate_pval_from_gammafit_to_trials(
            ts_vals, ts_threshold, eta=eta, n_max=n_max
        )
...
def calculate_critical_ts_from_gamma(ts, h0_ts_quantile, eta=3.0):
    ...
    norm = alpha / gamma.sf(eta, a=pars[0], scale=pars[1])
    critical_ts = gamma.ppf(
        1 - 1.0 / norm * h0_ts_quantile, a=pars[0], scale=pars[1]
    )
...
def estimate_mean_nsignal_for_ts_quantile(  # noqa: C901
    ...
    if (critical_ts is None) and (h0_ts_quantile is None):
        ...
        n_trials_min = int(
            h0_ts_quantile * (1 - h0_ts_quantile) / eps**2 + 0.5
        )
        ...
        logger.debug(
            "Critical ts value for bkg ts quantile %g: %e", h0_ts_quantile, c
        )
...
def generate_mu_of_p_spline_interpolation(
    ...
    logger.debug(
        ...
        len(h0_ts_vals),
        (len(h0_ts_vals) / n_h0_ts_vals) * 100,
    )

    c = np.percentile(h0_ts_vals, (1 - h0_ts_quantile) * 100)
    logger.debug(
        "Critical ts value for bkg ts quantile %g: %g", h0_ts_quantile, c
    )
    ...
    spline = make_spline_1d(
        p_vals, mu_vals, kind=kind, copy=False, assume_sorted=True
    )
...
def create_trial_data_file(  # noqa: C901
    ...
    """
    n_trials = int_cast(
        n_trials, "The n_trials argument must be castable to type int!"
    )
        ...
        if not issequence(mean_n_sig_null):
            ...
            if len(mean_n_sig_null) == 2:
                ...
                (
                    mean_n_sig_null_min,
                    mean_n_sig_null_max,
                    mean_n_sig_null_step,
                ) = mean_n_sig_null

        mean_n_sig_null = np.arange(
            mean_n_sig_null_min,
            mean_n_sig_null_max + 1,
            mean_n_sig_null_step,
            dtype=np.float64,
        )

    pbar = ProgressBar(
        len(mean_n_sig) * len(mean_n_sig_null), parent=ppbar
    ).start()
    trial_data = None
...
def calculate_upper_limit_distribution(
    ana, rss, pathfilename, n_bkg=5000, n_bins=100
):
