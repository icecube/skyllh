...
def rotate_spherical_vector(ra1, dec1, ra2, dec2, ra3, dec3):
    ...
    cos_alpha = np.cos(ra2 - ra1) * np.cos(dec1) * np.cos(dec2) + np.sin(
        dec1
    ) * np.sin(dec2)
    ...
    for i in range(N_event):
        ...
        skv = np_roll(
            np_roll(np_diag(nrot_i), shift=1, axis=1), shift=-1, axis=0
        )
        nrotx_i = skv - np_T(skv)

        # Calculate rotation matrix, R_i.
        R_i = (
            (1.0 - cos_alpha_i) * nrotTnrot_i
            + one * cos_alpha_i
            + sin_alpha[i] * nrotx_i
        )
...
def angular_separation(ra1, dec1, ra2, dec2, psi_floor=None):
    ...
    x = (
        np.sin(delta_dec / 2.0) ** 2.0
        + np.cos(dec1) * np.cos(dec2) * np.sin(delta_ra / 2.0) ** 2.0
    )
