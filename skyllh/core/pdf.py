...
class PDFAxis:
    ...
    def __init__(self, name: str, vmin: float, vmax: float, *args, **kwargs):
    ...
    @vmin.setter
    def vmin(self, v):
        self._vmin = float_cast(v, 'The value for the vmin property must be cast-able to type float!')
    ...
    @vmax.setter
    def vmax(self, v):
        self._vmax = float_cast(v, 'The value for the vmax property must be cast-able to type float!')
    ...
    def __eq__(self, other):
        ...
        """
        return (self.name == other.name) and np.isclose(self.vmin, other.vmin) and np.isclose(self.vmax, other.vmax)

    def __str__(self):
        """Pretty string implementation for the PDFAxis instance."""
        s = f'{classname(self)}: {self._name}: ' + f'vmin={self._vmin:g} vmax={self._vmax:g}'
        return s
...
class PDFAxes(NamedObjectCollection):
    ...
    def union(*axeses) -> 'PDFAxes':
        ...
        *axeses : PDFAxes instances
        ...
        if not issequenceof(axeses, PDFAxes):
            raise TypeError('The arguments of the union static function must be instances of PDFAxes!')
        if not len(axeses) >= 1:
            raise ValueError('At least 1 PDFAxes instance must be provided to the union static function!')
    ...
    def __init__(self, axes: 'Sequence[PDFAxis] | None' = None, **kwargs):
        ...
        """
        super().__init__(objs=axes, obj_type=PDFAxis, **kwargs)

    def __str__(self):
        """Pretty string implementation for the PDFAxes instance."""
        return '\n'.join(str(axis) for axis in self)

    def is_same_as(self, axes: 'PDFAxes | Sequence[PDFAxis]') -> bool:
        ...
        return all(self_axis == axes_axis for self_axis, axes_axis in zip(self, axes, strict=True))
...
class IsBackgroundPDF:
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not isinstance(self, PDF):
            raise TypeError(f'The class "{classname(self)}" is not derived from PDF!')

        super().__init__(*args, **kwargs)

    def __mul__(self, other: 'IsBackgroundPDF'):
        ...
        if not isinstance(other, IsBackgroundPDF):
            raise TypeError('The other PDF must be an instance of IsBackgroundPDF!')

        _self = cast('PDF', self)
        return BackgroundPDFProduct(_self, cast('PDF', other), cfg=cast(HasConfig, self).cfg)
...
class IsSignalPDF:
    ...
    def __mul__(self, other: 'IsSignalPDF'):
        ...
        if not isinstance(other, IsSignalPDF):
            raise TypeError('The other PDF must be an instance of IsSignalPDF!')

        return SignalPDFProduct(cast('PDF', self), cast('PDF', other), cfg=cast(HasConfig, self).cfg)
...
class PDF(
    ...
    def __init__(
        self,
        pmm: ParameterModelMapper | None = None,
        param_set: Parameter | Sequence[Parameter] | ParameterSet | None = None,
        **kwargs,
    ):
    ...
    @pmm.setter
    def pmm(self, mapper):
        if mapper is not None and not isinstance(mapper, ParameterModelMapper):
            raise TypeError(
                f'The pmm property must be an instance of ParameterModelMapper! Its type is "{classname(mapper)}"!'
            )
        self._pmm = mapper
...
class PDFProduct(
    ...
    def __init__(self, pdf1: 'PDF', pdf2: 'PDF', **kwargs):
        ...
        if pdf1.pmm is not pdf2.pmm:
            raise ValueError('The ParameterModelMapper instance of pdf1 is not the same as for pdf2!')

        # Create the ParameterSet instance that is the union of the ParameterSet
        # instances of the two PDFs.
        param_set = ParameterSet.union(self._pdf1.param_set, self._pdf2.param_set)

        super().__init__(pmm=pdf1.pmm, param_set=param_set, **kwargs)
    ...
    def assert_is_valid_for_trial_data(self, tdm: TrialDataManager, tl: TimeLord | None = None, **kwargs):
        ...
        """
        self._pdf1.assert_is_valid_for_trial_data(tdm=tdm, tl=tl, **kwargs)

        self._pdf2.assert_is_valid_for_trial_data(tdm=tdm, tl=tl, **kwargs)
    ...
    def get_pd(
        ...
        with TaskTimer(tl, f'Get probability densities from {classname(pdf1)} (pdf1) and {classname(pdf2)} (pdf2).'):
            (pd1, grads1) = pdf1.get_pd(tdm=tdm, params_recarray=params_recarray, tl=tl)
            (pd2, grads2) = pdf2.get_pd(tdm=tdm, params_recarray=params_recarray, tl=tl)
...
class SignalPDFProduct(PDFProduct, IsSignalPDF):
    ...
    def __init__(self, pdf1, pdf2, **kwargs):
        ...
        super().__init__(pdf1=pdf1, pdf2=pdf2, **kwargs)
...
class SpatialPDF(PDF, metaclass=abc.ABCMeta):
    ...
    def __init__(self, ra_range: tuple, dec_range: tuple, **kwargs):
        ...
        self.add_axis(PDFAxis(name='ra', vmin=ra_range[0], vmax=ra_range[1]))
        self.add_axis(PDFAxis(name='dec', vmin=dec_range[0], vmax=dec_range[1]))
    ...
    def assert_is_valid_for_trial_data(self, tdm: TrialDataManager, tl: TimeLord | None = None, **kwargs):
        ...
        if np.any((ra < ra_axis.vmin) | (ra > ra_axis.vmax)):
            raise ValueError(
                f'Some data is outside the right-ascension range ({ra_axis.vmin:.3f}, {ra_axis.vmax:.3f})!'
            )

        # Check if all the data is within the declination range.
        if np.any((dec < dec_axis.vmin) | (dec > dec_axis.vmax)):
            raise ValueError(f'Some data is outside the declination range ({dec_axis.vmin:.3f}, {dec_axis.vmax:.3f})!')
...
class TimePDF(PDF, metaclass=abc.ABCMeta):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.add_axis(PDFAxis(name='time', vmin=self._livetime.time_window[0], vmax=self._livetime.time_window[1]))
    ...
    def __str__(self):
        ...
        s = (
            f'{classname(self)}(\n'
            ' ' * INDENTATION_WIDTH + f'livetime = {self._livetime!s},\n'
            ' ' * INDENTATION_WIDTH + f'time_flux_profile = {self._time_flux_profile!s}\n'
            ')'
        )

        return s

    def _calculate_sum_of_ontime_time_flux_profile_integrals(self) -> float:
        ...
        uptime_intervals = self._livetime.get_uptime_intervals_between(
            self._time_flux_profile.t_start, self._time_flux_profile.t_stop
        )

        S = np.sum(self._time_flux_profile.get_integral(uptime_intervals[:, 0], uptime_intervals[:, 1]))
    ...
    def assert_is_valid_for_trial_data(self, tdm: TrialDataManager, tl: TimeLord | None = None, **kwargs):
        ...
        if np.any((time < time_axis.vmin) | (time > time_axis.vmax)):
            raise ValueError(f'Some trial data is outside the time range [{time_axis.vmin:.3f}, {time_axis.vmax:.3f}]!')
...
class MultiDimGridPDF(
    ...
    def __init__(
        ...
        axis_binnings: BinningDefinition | Sequence[BinningDefinition],
        path_to_pdf_splinetable: str | None = None,
        pdf_grid_data: np.ndarray | None = None,
        ...
        **kwargs,
        ...
        elif path_to_pdf_splinetable is not None and pdf_grid_data is not None:
            raise ValueError(
                'Only one of the two arguments path_to_pdf_splinetable and pdf_grid_data can be specified!'
            )
        ...
        if path_to_pdf_splinetable is not None:
            ...
            if not tool.is_available('photospline'):
                raise ImportError(
                    'The path_to_pdf_splinetable argument is specified, but the "photospline" package is not available!'
                )

            tool.assert_tool_version('photospline', '>=2.2.0')

        if pdf_grid_data is not None and not isinstance(pdf_grid_data, np.ndarray):
            raise TypeError(
        ...
        for axis_binning in self._axis_binning_list:
            self.add_axis(PDFAxis(name=axis_binning.name, vmin=axis_binning.lower_edge, vmax=axis_binning.upper_edge))
        ...
        if path_to_pdf_splinetable is None:
            ...
            self._pdf = tool.get('photospline').SplineTable(path_to_pdf_splinetable)
        ...
        logger.debug(f'Created {classname(self)} instance with axis name list {self._axes.name_list!s}')
    ...
    def norm_factor_func(self, func):
        if func is None:
            ...
            func = _unity_norm_func

        if not callable(func):
            raise TypeError('The norm_factor_func property must be a callable object!')
        if not func_has_n_args(func, 5):
            raise TypeError('The norm_factor_func property must be a function with 5 arguments!')
    ...
    def cache_pd_values(self, b):
        self._cache_pd_values = bool_cast(b, 'The cache_pd_values property must be cast-able to type bool!')
    ...
    def _store_pd_values_to_cache(
        ...
        if self._cache_pd is None:
            self._cache_pd = np.full(pd.shape, np.nan, dtype=np.float64)
    ...
    def _get_cached_pd_values(self, tdm: TrialDataManager, evt_mask: np.ndarray | None = None) -> np.ndarray | None:
        ...
        """
        if (
            self._cache_tdm_trial_data_state_id is None
            or self._cache_tdm_trial_data_state_id != tdm.trial_data_state_id
        ):
        ...
        if evt_mask is None:
            ...
            assert self._cache_pd is not None
    ...
    def get_pd_with_eventdata(
        ...
        if self._cache_pd_values:
            pd = self._get_cached_pd_values(tdm=tdm, evt_mask=evt_mask)
            if pd is not None:
                return pd
        ...
        with TaskTimer(tl, 'Normalize MultiDimGridPDF with norm factor.'):
            norm = self._norm_factor_func(
                pdf=self, tdm=tdm, params_recarray=params_recarray, eventdata=eventdata, evt_mask=evt_mask
            )

            pd *= norm

        if self._cache_pd_values:
            self._store_pd_values_to_cache(tdm=tdm, pd=pd, evt_mask=evt_mask)
    ...
    def get_pd(
        ...
        with TaskTimer(tl, 'Get PDF eventdata.'):
            _axes = cast('PDFAxes', self._axes)
            if self.is_signal_pdf:
                eventdata = self.create_eventdata_for_sigpdf(tdm=tdm, axes=_axes)
            elif self.is_background_pdf:
                eventdata = self.create_eventdata_for_bkgpdf(tdm=tdm, axes=_axes)
            else:
                raise TypeError('The PDF is neither a signal nor a background PDF!')
...
class PDFSet(
    ...
    def __init__(self, param_grid_set: ParameterGrid | ParameterGridSet, **kwargs):
    ...
    def param_grid_set(self, obj):
        ...
        if obj is not None and not isinstance(obj, ParameterGridSet):
            raise TypeError('The params_grid_set property must be an instance of type ParameterGridSet!')
    ...
    def __contains__(self, key: dict | int):
        ...
        if not isinstance(key, int):
            raise TypeError(f'The key argument must be of type dict or int! currently its type is {classname(key)}.')
    ...
    def make_key(self, gridparams: dict) -> int:
    ...
    def add_pdf(self, pdf: 'PDF', gridparams: dict):
        ...
        if not isinstance(pdf, PDF):
            raise TypeError(f'The pdf argument must be an instance of PDF!But its type is "{classname(pdf)}!')
        if not isinstance(gridparams, dict):
            raise TypeError(f'The gridparams argument must be of type dict!But its type is "{classname(gridparams)}"!')

        gridparams_hash = make_dict_hash(gridparams)
        if gridparams_hash in self._gridparams_hash_pdf_dict:
            raise KeyError(f'The PDF with grid parameters {gridparams!s} was already added!')

        # Check that the new PDF has the same axes than the already added PDFs.
        if len(self._gridparams_hash_pdf_dict) > 0:
            some_pdf = self._gridparams_hash_pdf_dict[next(iter(self._gridparams_hash_pdf_dict.keys()))]
    ...
    def get_pdf(self, gridparams: dict | int):
        ...
        if isinstance(gridparams, int):
            ...
            raise TypeError('The gridparams argument must be of type dict or int!')

        if gridparams_hash not in self._gridparams_hash_pdf_dict:
            raise KeyError(f'No PDF was created for the parameter set "{gridparams!s}"!')
    ...
    def initialize_for_new_trial(self, tdm: TrialDataManager, tl: TimeLord | None = None, **kwargs):
        ...
        for pdf in self._gridparams_hash_pdf_dict.values():
            pdf.initialize_for_new_trial(tdm=tdm, tl=tl, **kwargs)
    ...
    def assert_is_valid_for_trial_data(self, tdm: TrialDataManager, tl: TimeLord | None = None, **kwargs):
        ...
        pdf.assert_is_valid_for_trial_data(tdm=tdm, tl=tl, **kwargs)

    def get_pd(
        self, gridparams: dict, tdm: TrialDataManager, params_recarray: np.ndarray | None = None, tl=None
    ) -> 'tuple[np.ndarray, dict]':
        ...
        return pdf.get_pd(tdm=tdm, params_recarray=params_recarray, tl=tl)
