...
class LLHRatio(
    ...
    def minimizer(self, minimizer):
        if not isinstance(minimizer, Minimizer):
            raise TypeError(
                f'The minimizer property must be an instance of Minimizer! Its current type is {classname(minimizer)}.'
            )
    ...
    def maximize(self, rss: RandomStateService, tl: TimeLord | None = None) -> tuple[float, np.ndarray, dict]:
        ...
        def negative_llhratio_func(fitparam_values, func_stats, tl=None):
            ...
            with TaskTimer(tl, 'Evaluate llh-ratio function.'):
                ...
                if tracing:
                    logger.debug(f'LLH-ratio func value f={f:g}, grads={grads!s}')
            return (-f, -grads)
        ...
        logger.debug(f'Maximized LLH ratio function with {status["n_llhratio_func_calls"]:d} calls')
...
class TCLLHRatio(LLHRatio, metaclass=abc.ABCMeta):
    ...
    def mean_n_sig_0(self, v):
        v = float_cast(v, 'The mean_n_sig_0 property must be cast-able to a float value!')
    ...
    def maximize_with_1d_newton_rapson_minimizer(
        ...
        def negative_llhratio_func_nr1d_ns(fitparam_values, tl):
            ...
            with TaskTimer(tl, 'Calculate 2nd derivative of llh-ratio function w.r.t. ns'):
                grad2_ns = self__calculate_ns_grad2(
                    ns=ns, ns_pidx=ns_pidx, src_params_recarray=src_params_recarray, tl=tl
                )

            return (-f, -grads[ns_pidx], -grad2_ns)

        (fitparam_values, fmin, status) = self._minimizer.minimize(
            rss=rss, paramset=self._pmm.global_paramset, func=negative_llhratio_func_nr1d_ns, args=(tl,)
        )
        log_lambda_max = -fmin
    ...
    def maximize(self, rss: RandomStateService, tl: TimeLord | None = None) -> tuple[float, np.ndarray, dict]:
        ...
        status
            ...
            process, i.e. from the minimizer.
        """
        if isinstance(self._minimizer.minimizer_impl, (NR1dNsMinimizerImpl, NRNsScan2dMinimizerImpl)):
            return self.maximize_with_1d_newton_rapson_minimizer(rss=rss, tl=tl)

        return super().maximize(rss=rss, tl=tl)
...
class SingleDatasetTCLLHRatio(TCLLHRatio, metaclass=abc.ABCMeta):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._tdm.calculate_source_data_fields(shg_mgr=self._shg_mgr, pmm=self._pmm)
    ...
    def tdm(self, mgr):
        if not isinstance(mgr, TrialDataManager):
            raise TypeError(
                f'The tdm property must be an instance of TrialDataManager! Its current type is {classname(mgr)}.'
            )
        self._tdm = mgr
    ...
    def change_shg_mgr(self, shg_mgr: SourceHypoGroupManager):
        ...
        self._tdm.change_shg_mgr(shg_mgr=shg_mgr, pmm=self._pmm)
...
class ZeroSigH0SingleDatasetTCLLHRatio(SingleDatasetTCLLHRatio):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        super().__init__(pmm=pmm, minimizer=minimizer, shg_mgr=shg_mgr, tdm=tdm, mean_n_sig_0=0, **kwargs)
    ...
    def pdfratio(self, r):
        if not isinstance(r, PDFRatio):
            raise TypeError(
                f'The pdfratio property must be an instance of PDFRatio! Its current type is {classname(r)}.'
            )
        self._pdfratio = r
    ...
    def calculate_log_lambda_and_grads(
        ...
        if tracing:
            logger.debug(f'N={N:d}, Nprime={Nprime:d}')
        ...
        if tracing:
            logger.debug(f'# of events doing Taylor expansion for (unstable events): {np.count_nonzero(m_unstable):d}')
        ...
        nsgrad_i[m_stable] = Xi[m_stable] * one_over_one_plus_alpha_i_stablemask
        if any_unstable_events:
        ...
        grads[p_mask] = np.sum(ns * one_over_one_plus_alpha_i_stablemask[:, np.newaxis] * dXi_dp[m_stable], axis=0)

        # For all numerical unstable events.
        if any_unstable_events:
            grads[p_mask] += np.sum(
                ns * (1 - tildealpha_i[:, np.newaxis]) * dXi_dp[m_unstable] / one_plus_alpha, axis=0
            )
    ...
    def calculate_ns_grad2(  # type: ignore[override]
        ...
        if self._cache_nsgrad_i is None:
            raise RuntimeError(
                'The evaluate method needs to be called before the calculate_ns_grad2 method can be called!'
            )
    ...
    def evaluate(
        ...
        with TaskTimer(tl, 'Calc pdfratio value Ri'):
            Ri = self._pdfratio.get_ratio(tdm=tdm, src_params_recarray=src_params_recarray, tl=tl)
        ...
        dXi_dp = np.empty((Xi.shape[0], n_fitparams - 1), dtype=np.float64)
        ...
        if tracing:
            logger.debug(f'{classname(self)}.evaluate: N={N}, Nprime={len(Xi)}, ns={ns:.3f}')

        with TaskTimer(tl, 'Calc logLambda and grads'):
            (log_lambda, grads) = self.calculate_log_lambda_and_grads(
                N=N, ns=ns, ns_pidx=ns_pidx, p_mask=p_mask, Xi=Xi, dXi_dp=dXi_dp
            )
...
class MultiDatasetTCLLHRatio(TCLLHRatio):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(pmm=pmm, minimizer=minimizer, mean_n_sig_0=self._llhratio_list[0].mean_n_sig_0, **kwargs)
        ...
        if self.ds_sig_weight_factors_service.n_datasets != len(self._llhratio_list):
            raise ValueError(
    ...
    def evaluate(
        ...
        ns = fitparam_values[ns_pidx]
        if tracing:
            logger.debug(f'{classname(self)}.evaluate: ns={ns:.3f}')

        # We need to calculate the source detsigyield weights and the dataset
        # signal weight factors.
        self._src_detsigyield_weights_service.calculate(src_params_recarray=src_params_recarray)
        ...
        for j, llhratio in enumerate(self._llhratio_list):
            if tracing:
                logger.debug(f'nsf[j={j}] = {nsf[j]:.3f}')

            llhratio_fitparam_values[ns_pidx] = nsf[j]

            (log_lambda_j, grads_j) = llhratio.evaluate(
                fitparam_values=llhratio_fitparam_values, src_params_recarray=src_params_recarray, tl=tl
            )
    ...
    def calculate_ns_grad2(  # type: ignore[override]
        ...
        for j, llhratio in enumerate(self._llhratio_list):
            nsgrad2j[j] = llhratio.calculate_ns_grad2(
                ns=nsf[j], ns_pidx=ns_pidx, src_params_recarray=src_params_recarray, tl=tl
            )
...
class NsProfileMultiDatasetTCLLHRatio(TCLLHRatio):
    ...
    def initialize_for_new_trial(self, tl: TimeLord | None = None, **kwargs):
        ...
        tl
            ...
            measurements.
        """
        self._llhratio.initialize_for_new_trial(tl=tl, **kwargs)
    ...
    def calculate_ns_grad2(  # type: ignore[override]
        ...
        """
        if ns_pidx != 0:
            raise ValueError(f'The value of the ns_pidx argument must be 0! Its current value is {ns_pidx}.')
