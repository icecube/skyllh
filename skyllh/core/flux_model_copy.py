...
class PhotosplineDMEnergyFluxProfile(
    ...
    @channel.setter
    def channel(self,c_value):
        if not c_value in ['WW', 'bb', 'numunumu','nuenue']:
            raise ValueError( 'not an available annahilation channel!')
        self._channel = c_value
...
#             raise ValueError('the precision of mass float number does not fulfill requirement')
    ...
    def get_nevents_from_source(self, mc, src_dec, neutrino_flux, ltime=8.7, dec_bw=0.5):
