...
class I3Dataset(
    ...
    def grl_field_name_renaming_dict(self, d):
        if not isinstance(d, dict):
            raise TypeError('The grl_field_name_renaming_dict property must be an instance of dict!')
    ...
    def exists(self):
        ...
        return all(os.path.exists(pathfilename) for pathfilename in self.grl_abs_pathfilename_list)
            ...
            if ('run' in data.grl) and ('run' in data.exp):
                ...
                with TaskTimer(tl, task):
                    ...
                    if np.any(~mask):
                        ...
                        self._logger.info(
                            f'Cutting {n_cut_runs} runs from dataset {self.name} due to GRL run information.'
                        )
                        data.exp = data.exp[mask]

            # Select based on detector on-time information.
            if ('start' in data.grl) and ('stop' in data.grl) and ('time' in data.exp):
                task = (
...
class I3DatasetData(
    ...
    def __init__(
        ...
        """
        super().__init__(data_exp=data._exp, data_mc=data._mc, livetime=data._livetime)
