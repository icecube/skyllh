...
class SignalI3EnergyPDFSet(PDFSet, IsSignalPDF, IsParallelizable):
    ...
    def __init__(self, data_mc, logE_binning, sinDec_binning, fluxmodel,
        ...
        def create_I3EnergyPDF(
            ...
            myfluxmodel = fluxmodel.copy(newparams=gridfitparams)
            ...
            i3energypdf = I3EnergyPDF(
                pmm=None,
                data_log_energy=data_logE,
                data_sin_dec=data_sinDec,
                data_mcweight=data_mcweight,
                data_physicsweight=data_physicsweight,
                log_energy_binning=logE_binning,
                sin_dec_binning=sinDec_binning,
        ...
        for (gridfitparams, i3energypdf) in zip(self.gridfitparams_list,
                                                i3energypdf_list):
            self.add_pdf(i3energypdf, gridfitparams)

    def assert_is_valid_for_trial_data(self, tdm):
        """Checks if this signal energy PDF is valid for all the given trial
        data.
        ...
        self.get_pdf(self.pdf_keys[0]).assert_is_valid_for_trial_data(tdm=tdm)

    def get_pd(self, tdm, gridparams, tl=None):
        ...
        gridparams : dict
            ...
            SignalI3EnergyPDFSet instance. There is no interpolation method
            defined at this point to allow for arbitrary parameter values!
        ...
        """
        i3energypdf = self.get_pdf(gridparams)

        pd = i3energypdf.get_pd(tdm)
        return pd
