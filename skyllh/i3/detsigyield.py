...
class I3DetSigYield(DetSigYield):
    ...
    def __init__(self, implmethod, dataset, fluxmodel, livetime, sin_dec_binning):
        ...
        class.
        """
        super(I3DetSigYield, self).__init__(
            implmethod, dataset, fluxmodel, livetime)
...
class PointLikeSourceI3DetSigYieldImplMethod(I3DetSigYieldImplMethod):
    ...
    def source_to_array(self, sources):
        ...
        if(not issequenceof(sources, PointLikeSource)):
            raise TypeError(
                'The source argument must be an instance of PointLikeSource!')
...
class FixedFluxPointLikeSourceI3DetSigYieldImplMethod(
    ...
    def construct_detsigyield(self, dataset, data, fluxmodel, livetime, ppbar=None):
        ...
        w = data.mc["mcweight"] * \
            fluxmodel(data.mc["true_energy"])*toGeVcm2s * livetime_days * \
            86400.
